...
from vint_train.models.fastnav.self_attention import PositionalEncoding
from vint_train.models.fastnav.reweighting import TFDR
from vint_train.models.fastnav.temp_dilated_attn import DilateAttentionToken, MultiDilateTokenAttention
...
class NoMaD_ViNT(nn.Module):
    def __init__(
        ...
        """
        NoMaD_ViNT model has been adapted for our FastNav.

        This class is built on top of the original NoMaD model, with modifications to:
        - Support FastNav's RGB-only image-goal navigation
        - Integrate temporal-dilated attention for multi-scale temporal dependencies
        - Include temporal-feature decoupled reweighting for navigational context enhancement
        """
        super().__init__()
        ...
        # Replace Standard Self-Attention with Two Temporal-Dilated Attention Layers
        self.dilation_attn_layer1 = MultiDilateTokenAttention()
        self.dilation_attn_layer2 = MultiDilateTokenAttention()

        self.reweighting = TFDR()
    ...
    def forward(self, obs_img: torch.tensor, goal_img: torch.tensor, input_goal_mask: torch.tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        obs_encoding_tokens = self.dilation_attn_layer1(obs_encoding)
        obs_encoding_tokens = self.dilation_attn_layer2(obs_encoding_tokens)
        obs_encoding_tokens = self.reweighting(obs_encoding_tokens)
        ...
        # print("obs_encoding_tokens.shape: ", obs_encoding_tokens.shape)
